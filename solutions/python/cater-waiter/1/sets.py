...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if ingredientes.issubset(VEGETARIAN):
        return f"{dish_name}: VEGETARIAN"
    if ingredientes.issubset(PALEO):
        return f"{dish_name}: PALEO"
    if ingredientes.issubset(KETO):
...
def tag_special_ingredients(dish):
    ...
    """
    platillo, ingredientes = dish
    alergias = set(ingredientes).intersection(SPECIAL_INGREDIENTS)
    return(platillo, alergias)
...
def singleton_ingredients(dishes, intersection):
    ...
    return ingredientes - intersection
